...
WinDivert.sys является kernel-mode драйвером. При первом запуске службы WinDivert автоматически регистрирует встроенный `WinDivert64.sys`; требуется запуск установщика/службы с правами администратора и разрешённая Windows подпись драйвера. Не отключайте Secure Boot и проверку подписи на пользовательских системах. Перед распространением подпишите приложение, службу и установщик сертификатом издателя.
