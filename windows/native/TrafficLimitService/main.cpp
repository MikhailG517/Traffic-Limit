...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR command,int){std::wstring args=command?command:L"";if(args.find(L"--install")!=std::wstring::npos)return InstallService()?0:1;if(args.find(L"--uninstall")!=std::wstring::npos)return UninstallService()?0:1;if(args.find(L"--set-limit")!=std::wstring::npos)return SendLimit(args);SERVICE_TABLE_ENTRYW table[]={{const_cast<LPWSTR>(L"TrafficLimitService"),ServiceMain},{nullptr,nullptr}};return StartServiceCtrlDispatcherW(table)?0:1;}
