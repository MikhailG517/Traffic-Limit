...
static bool InstallService(){
  ...
  SC_HANDLE service=CreateServiceW(manager,L"TrafficLimitService",L"Traffic Limit Service",SERVICE_ALL_ACCESS,SERVICE_WIN32_OWN_PROCESS,SERVICE_AUTO_START,SERVICE_ERROR_NORMAL,path,nullptr,nullptr,nullptr,nullptr,nullptr);
...
static int SendLimit(const std::wstring& args){HANDLE pipe=CreateFileW(L"\\\\.\\pipe\\TrafficLimit",GENERIC_READ|GENERIC_WRITE,0,nullptr,OPEN_EXISTING,0,nullptr);if(pipe==INVALID_HANDLE_VALUE)return 1;std::string payload(args.begin(),args.end()); DWORD written=0;WriteFile(pipe,payload.data(),(DWORD)payload.size(),&written,nullptr);char response[512]{};DWORD read=0;ReadFile(pipe,response,sizeof(response)-1,&read,nullptr);response[read]=0;DWORD out=0;WriteFile(GetStdHandle(STD_OUTPUT_HANDLE),response,read,&out,nullptr);CloseHandle(pipe);return 0;}
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR command,int){std::wstring args=command?command:L"";if(args.find(L"--install")!=std::wstring::npos)return InstallService()?0:1;if(args.find(L"--uninstall")!=std::wstring::npos)return UninstallService()?0:1;if(args.find(L"--set-limit")!=std::wstring::npos||args.find(L"--get-processes")!=std::wstring::npos||args.find(L"--disable")!=std::wstring::npos)return SendLimit(args);SERVICE_TABLE_ENTRYW table[]={{const_cast<LPWSTR>(L"TrafficLimitService"),ServiceMain},{nullptr,nullptr}};return StartServiceCtrlDispatcherW(table)?0:1;}
